import { BaseCommandInteraction, Client, ClientOptions, Intents, Interaction } from "discord.js";
...
export default class Mao extends Client {
    private static instance: Mao;

    static defaultPrefix: string = "m.";

    private commands: Record<string, Command> = {};

    constructor(options?: ClientOptions) {
        if (Mao.instance) throw new Error("can't instantiate more than 1 Mao");
        super(
            options ?? {
                intents: [
        ...
        this.on("interactionCreate", async (event: Interaction) => {
            if (event.isCommand()) this.handleSlashCommand(event);
        });
    ...
    async handleSlashCommand(command: BaseCommandInteraction) {
